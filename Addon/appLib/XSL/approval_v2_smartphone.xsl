...
-->
	<xsl:template match="RRDOFFICE" >
		<xsl:if test="OFFICEDOCID[text()] != '' " >
			<span style="color:#FF0000">※Excel申請内容があります（この端末からは参照できません）</span>
			<br/>
		</xsl:if>
	</xsl:template>
...
<!-- StraForm域のテンプレート -->
	<xsl:template match="externalDocument">
		<xsl:if test="documentId[text()] != '' and formId[text()] != ''" >
			<span style="color:#FF0000">※StraForm申請内容があります（この端末からは参照できません）</span>
			<br/>
		</xsl:if>
	</xsl:template>
...
<!-- attachment, paperDocument -->
	<xsl:template match="RRDATTACHMENTCOMMENT">
		<xsl:if test="string-length(./text())>0">
				<span style="color:#FF0000">[紙添付資料]</span>
				<br/>
				<xsl:value-of select="." disable-output-escaping="yes" />
				<br/>
		</xsl:if>
		...
		<xsl:if test="string-length(./text())>0">
			...
			<xsl:choose>
				<xsl:when test="../RRDBBS">
					<span style="color:#FF0000">[掲載責任者]</span>
					<br/>
					<xsl:value-of select="./CHARGEEMPNAME" disable-output-escaping="yes" />
					<br/>
					<span style="color:#FF0000">[掲示期間]</span>
					<br/>
					<xsl:value-of select="./DISPSTARTDATE" disable-output-escaping="yes" />～<xsl:value-of select="./DISPENDDATE" disable-output-escaping="yes" />
					<br/>
					<span style="color:#FF0000">[NaviView通知期間]</span>
					<br/>
					<xsl:choose>
						...
						<xsl:when test="./NAVIVIEWDISPON">
							...
							<xsl:choose>
								<xsl:when test="./NAVIBBSDISPSTARTDATE">
									<span style="color:#FF0000">[通知期間]</span>
									<br/>
									<xsl:value-of select="./NAVIBBSDISPSTARTDATE" disable-output-escaping="yes" />～<xsl:value-of select="./NAVIBBSDISPENDDATE" disable-output-escaping="yes" />
									<br/>
								</xsl:when>
							</xsl:choose>
							...
							<xsl:choose>
								<xsl:when test="./NAVIEVENTDISPSTARTDATE">
									<span style="color:#FF0000">[通知期間]</span>
									<br/>
									<xsl:value-of select="./NAVIEVENTDISPSTARTDATE" disable-output-escaping="yes" />～<xsl:value-of select="./NAVIEVENTDISPENDDATE" disable-output-escaping="yes" />
									<br/>
									<span style="color:#FF0000">[開催期間]</span>
									<br/>
									<xsl:choose>
										<xsl:when test="./HOLDINGFROMDATE">
											<xsl:value-of select="./HOLDINGFROMDATE" disable-output-escaping="yes" />～<xsl:value-of select="./HOLDINGTODATE" disable-output-escaping="yes" />
										</xsl:when>
									</xsl:choose>
									<br/>
									<span style="color:#FF0000">[開催場所]</span>
									<br/>
					...
					<span style="color:#FF0000">[掲載掲示板]</span>
					<br/>
