...
-->
	<xsl:template match="RRDOFFICE" >
		<xsl:if test="OFFICEDOCID[text()] != '' " >
			<span style="color:#FF0000">※Excel有申请内容（此终端上不能查看）</span>
			<br/>
		</xsl:if>
	</xsl:template>
...
<!-- StraForm域のテンプレート -->
	<xsl:template match="externalDocument">
		<xsl:if test="documentId[text()] != '' and formId[text()] != ''" >
			<span style="color:#FF0000">※StraForm有申请内容（此终端上不能查看）</span>
			<br/>
		</xsl:if>
	</xsl:template>
...
<!-- attachment, paperDocument -->
	<xsl:template match="RRDATTACHMENTCOMMENT">
		<xsl:if test="string-length(./text())>0">
				<span style="color:#FF0000">[纸张附件资料]</span>
				<br/>
				<xsl:value-of select="." disable-output-escaping="yes" />
				<br/>
		</xsl:if>
		...
		<xsl:if test="string-length(./text())>0">
			...
			<xsl:choose>
				<xsl:when test="../RRDBBS">
					<span style="color:#FF0000">[登载负责人]</span>
					<br/>
					<xsl:value-of select="./CHARGEEMPNAME" disable-output-escaping="yes" />
					<br/>
					<span style="color:#FF0000">[登载期间]</span>
					<br/>
					...
					<br/>
					<span style="color:#FF0000">[NaviView通知期间]</span>
					<br/>
					<xsl:choose>
						...
						<xsl:when test="./NAVIVIEWDISPON">
							...
							<xsl:choose>
								<xsl:when test="./NAVIBBSDISPSTARTDATE">
									<span style="color:#FF0000">[通知期间]</span>
									<br/>
							...
							<xsl:choose>
								<xsl:when test="./NAVIEVENTDISPSTARTDATE">
									...
									<br/>
									<span style="color:#FF0000">[举办期间]</span>
									<br/>
									<xsl:choose>
									...
									<br/>
									<span style="color:#FF0000">[举办地点]</span>
									<br/>
					...
					<span style="color:#FF0000">[登载告示牌]</span>
					<br/>
